...
from .models import ncf_bacs_files_to_process, \
                    ...
                    ncf_bacs_file_signatures_processed, \
                    ncf_bacs_reasons, \
...
from core_agreement_crud.models import go_account_transaction_summary, go_agreement_querydetail
...
def app_process_bacs_files():
        ...
        if path.file_type == 'xml':
            ...
            for dirpath, directories, filenames in os.walk(wip_network_path_source):
                ...
                for filename in filenames:
                    ...
                    if (path.file_identifier in filename) and (filename.endswith(".xml")):
                        ...
                        try:
                            ...
                            if path.target_data_format == 'xml-01':
                                ...
                                # Check for Signature already processed
                                wip_process_file = False
                                for elem_message_id in root.iter('Signature'):
                                    wip_signature_ntext = str(elem_message_id.text[-1024:])
                                    wip_signature = '&GO_' + wip_signature_ntext
                                    # try:
                                    test_signature = ncf_bacs_file_signatures_processed.objects.filter(
                                        file_signature=wip_signature).first()
                                    if test_signature:
                                        wip_process_file = False
                                    else:
                                        ncf_bacs_file_signatures_processed.objects.create(file_signature=wip_signature)
                                        wip_process_file = True
                            ...
                            if path.target_data_format == 'xml-03':
                                ...
                                wip_process_file = False
                                for elem_message_id in root.iter('Signature'):
                                    wip_signature = elem_message_id.text[-4000:]
                                    try:
                                        ncf_bacs_file_signatures_processed.objects.get(file_signature=wip_signature)
                                    except:
                                        ncf_bacs_file_signatures_processed.objects.create(file_signature=wip_signature)
                                        wip_process_file = True
    ...
    for empty_ddic_agreement in empty_ddic_agreements:
        ...
        # Get Customer
        querydetail_extract = None
        querydetail_extract = go_agreement_querydetail.objects.filter(agreementnumber=wip_agreement_id).first()
        if querydetail_extract:
            wip_customername = querydetail_extract.customercompany
        else:
            wip_customername = 'NOT FOUND'


        empty_ddic_agreement.agreement_id = wip_agreement_id
        empty_ddic_agreement.customercompany = wip_customername
